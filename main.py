...
# utils.generar_musica(r"Juego memotest - alumnes\Juego memotest - alumnes\recursos\fondo.wav",5)
...
while esta_corriendo:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            pos = event.pos
            # SONIDO_CLICK.play()
            utils.generar_sonido(r"Juego memotest - alumnes\Juego memotest - alumnes\recursos\clic.wav",20).play(0)
            if tablero.detectar_colision(tablero_juego, pos) != None:
                utils.generar_sonido(r"Juego memotest - alumnes\Juego memotest - alumnes\recursos\voltear.wav",20).play(0)
                pass
                # SONIDO_VOLTEAR.play()
